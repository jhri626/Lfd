import torch
import numpy as np
from agent.utils.R2_functions import gvf_R2 
...
class NeuralNetwork(torch.nn.Module):
    ...
    def __init__(self, dim_state, dynamical_system_order, n_primitives, multi_motion, latent_gain_lower_limit,
        ...
        self.encoder3 = torch.nn.Linear(neurons_hidden_layers, latent_input_size) # fix neurons_hidden_layers -> latent_input_size
        self.norm_e_3 = torch.nn.LayerNorm(neurons_hidden_layers)
        # self.encoder4 = torch.nn.Linear(neurons_hidden_layers, latent_input_size) # what if I add hidden layer?
        ...
        self.decoder3_dx = torch.nn.Linear(neurons_hidden_layers, n_output) # fix it
        '''
        add one more layer for test
        '''
    ...
    def latent_dynamical_system(self, y_t, y_traj , primitive_type, dt):
        """
        ...
        y_dot_traj = torch.stack([y_dot_traj for _ in range(y_t.shape[0])], dim=0)
