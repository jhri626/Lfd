...
class DynamicalSystem():
    ...
    def get_latent_state(self, x_t=None, y_traj=None ,space='task'):
        ...
        if space == 'task':
            ...
            if y_traj == None:
                raise ValueError('Need y_traj for BCSDM.')
            _, y_t = self.transition_latent_system(y_traj)
            self.y_t['latent'] = y_t
        else:
    ...
    def transition_latent_system(self, y_t=None, y_traj=None):
        """
        ...
        # Get derivative
        dy_t_d = self.model.latent_dynamical_system(y_t, y_traj ,self.primitive_type,self.delta_t)
